...
*/
{
    ...
    *pSize = 0;
    ...
    if (numBytes)
    {
        pBuffer = (BYTE*)ExAllocatePool2(POOL_FLAG_NON_PAGED, numBytes, TAG);//ExAllocatePool2默认清零。
        if (NULL == pBuffer) {
            PrintEx(DPFLTR_IHVNETWORK_ID, DPFLTR_ERROR_LEVEL, "分配缓冲区失败，numBytes:%u", numBytes);
            return NULL;
        }
        ...
        if (pTemplateNBL)
        {
            ...
            for (UINT32 bytesCopied = 0; bytesCopied < numBytes && pNB; pNB = NET_BUFFER_NEXT_NB(pNB))
            {
                ...
                if (bytesNeeded)
                {
                    ...
                    //若数据本就连续，NdisGetDataBuffer返回内部指针(未写入存储)，此时才需手动拷贝。
                         ...
                         _Inout_opt_ __drv_freesMem(Mem) WSACMSGHDR * controlData)
{
    ...
    PrintEx(DPFLTR_IHVNETWORK_ID, DPFLTR_INFO_LEVEL, "info:free DataGram packet:%p", packet);
}


void FreeTCPPendedPacket(_Inout_ __drv_freesMem(Mem) PPENDED_PACKET packet,
                         _Inout_opt_ __drv_freesMem(Mem) WSACMSGHDR * controlData)
{
    ...
    PrintEx(DPFLTR_IHVNETWORK_ID, DPFLTR_INFO_LEVEL, "info:free Stream packet:%p", packet);
}
...
void FreePendedPacket(_Inout_ __drv_freesMem(Mem) PPENDED_PACKET packet,
                      _Inout_opt_ __drv_freesMem(Mem) WSACMSGHDR * controlData)
{
    if (packet->belongingFlow->calloutId == g_CallOutId.STREAM_V4 ||
        packet->belongingFlow->calloutId == g_CallOutId.STREAM_V6) {
        FreeTCPPendedPacket(packet, controlData);
    } else if (packet->belongingFlow->calloutId == g_CallOutId.DATAGRAM_DATA_V4 ||
               packet->belongingFlow->calloutId == g_CallOutId.DATAGRAM_DATA_V6) {
        FreeUDPPendedPacket(packet, controlData);
    } else {
        PrintEx(DPFLTR_IHVNETWORK_ID, DPFLTR_WARNING_LEVEL, "warning: calloutId:%d", 
...
VOID NTAPI TCPInjectComplete(_In_ VOID * Context,
                             _Inout_ NET_BUFFER_LIST * NetBufferList,
                             _In_ BOOLEAN DispatchLevel)
{
    ...
    //FwpsDereferenceNetBufferList(NetBufferList, FALSE);
    FwpsFreeCloneNetBufferList(NetBufferList, 0);
}
...
再次注意:
不要再FwpsStreamInjectAsync的最后一个参数传递上下文结构,并在回调中释放内存.
这个问题搞了好几天,回调的释放时机和网络的操作和别的操作的机会难测.
*/
{
    NTSTATUS Status = STATUS_UNSUCCESSFUL;
    PFLOW_DATA FlowContext;

    if (NULL == packet) {
        return Status;
    }

    FlowContext = packet->belongingFlow;

    Status = FwpsStreamInjectAsync(g_Stream_InjectionHandle,
                                   NULL,
                                   0,
    ...
    if (!NT_SUCCESS(Status)) {
        /*
        If the return value is not STATUS_SUCCESS, the completion function will not be called.
        In this case, the network buffer list pointed to by metBufferList must be freed
        by a call to FwpsFreeNetBufferList0 or FwpsFreeCloneNetBufferList0.
        */

        /*
        这是一个经常失败的操作,如:网络延迟,调试器的断点停很长时间等
        */
    ...
    Status = STATUS_SUCCESS;//另一个思路是设置packet为0.
...
NTSTATUS UDPInboundInject(_In_ PPENDED_PACKET packet)
{
    ...
    if (ndisStatus != NDIS_STATUS_SUCCESS) {
        //回退失败(通常内存不足)，此时并未前移，不能再调用NdisAdvanceNetBufferDataStart，直接返回。
        PrintEx(DPFLTR_IHVNETWORK_ID, DPFLTR_ERROR_LEVEL, "error: NdisRetreatNetBufferDataStart失败，ndisStatus:%#x", ndisStatus);
...
NTSTATUS inject(PPENDED_PACKET packet)
{
    ...
    if (packet->belongingFlow->calloutId == g_CallOutId.STREAM_V4 ||
        ...
        PrintEx(DPFLTR_IHVNETWORK_ID, DPFLTR_ERROR_LEVEL, "error: calloutId:%#x", packet->belongingFlow->calloutId);
        //ASSERT(FALSE);
    }
...
*/
{
    ...
    if (packet->belongingFlow->size >= MAX_PATH * sizeof(WCHAR)) {
        ...
        PrintEx(DPFLTR_IHVNETWORK_ID, DPFLTR_WARNING_LEVEL, "warning: processPath:%wZ", temp);
    }
...
*/
{
    ...
    if (NULL == packet->KernelBuffer) {
        PrintEx(DPFLTR_IHVNETWORK_ID, DPFLTR_ERROR_LEVEL, "notice: DataLength: %d", (int)packet->DataLength);
        return;
    }
...
*/
{
    ...
    //放过和驱动通讯的进程自身的流量。
    //注意：本函数在工作线程(System进程上下文)执行，PsGetCurrentProcess恒为System，旧判断永远不成立(失效)。
    //改用流上下文采集的发起进程PID与通信进程PID比对，否则通信进程自身请求被送回它决策，可能死锁。
    ...
    SentToUser = ExAllocatePool2(POOL_FLAG_PAGED, sizeof(NOTIFICATION), TAG);//ExAllocatePool2默认清零。
    if (NULL == SentToUser) {
...
VOID WorkThread(_In_ PVOID StartContext)
{
    ...
    for (;;) {
        ...
        if (NULL != listEntry) {
            ...
            //等待新包入队(或卸载时被唤醒)，取代原来的1秒轮询，消除处理延迟。
    ...
    for (;;) {
        ...
        packet = CONTAINING_RECORD(listEntry, PENDED_PACKET, listEntry);
        FreePendedPacket(packet, packet->controlData);//在自旋锁外释放：FreePendedPacket会调Fwps*/释放池/DereferenceFlowContext。
    }

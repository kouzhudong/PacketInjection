...
*/
{
    ...
    //从ALE_FLOW_ESTABLISHED层采集的进程信息，通过flowContext传递过来。
    //这里增加引用，使belongingFlow中的指针(processPath/sid)指向该流上下文的缓存，
    //在packet释放前都有效，从而把进程信息正确上报给用户层。
    if (flowData != NULL) {
...
{
    PFLOW_DATA flowData = (PFLOW_DATA)flowContext;//由ALE_FLOW_ESTABLISHED层关联的上下文，可能为0。
    ...
    */
    ...
    KeSetEvent(&g_PacketEvent, IO_NO_INCREMENT, FALSE);//唤醒工作线程处理，取代轮询。
...
/*
释放对FLOW_DATA的一个引用。引用计数归为0时释放内存。
调用方：流被删除（FlowDeleteFn去掉链表引用）和缓存信息的每个pended packet（FreePendedPacket）。
注意：必须用InterlockedDecrement的返回值判0，不可“先减再读”，否则不是原子的。
*/
{
    if (NULL == flowData) {
        return;
...
VOID NTAPI FlowDeleteFn(IN UINT16 layerId, IN UINT32 calloutId, IN UINT64 flowContext)
{
    ...
    ReleaseFlowData(flowData);//释放链表引用；若仍有pended packet引用，最后一个再释放。
}
...
VOID AssociateOneContext(_In_ const FWPS_INCOMING_VALUES0 * pClassifyValues, _In_ const FWPS_INCOMING_METADATA_VALUES0 * pMetadata, UINT16 layerId, UINT32 calloutId)
{
    ...
    fc->refCount = 1;//链表引用。置于此后任何失败路径都可通过ReleaseFlowData统一释放。
    ...
    */
    ...
    if (!NT_SUCCESS(status)) {
        ...
        ReleaseFlowData(fc);//释放fc，包括processPath和sid。
        return;
    }
...
)
{
    ...
    //进程信息必须可用，AssociateOneContext会读取processPath->size/data，缺失则PERMIT返回。
...
*/
{
    ...
    for (; i < counter; i++) {
        if (temp[i]) {
            ...
            temp[i] = 0;//置零，使StopWFP可被重复调用（失败路径 + 卸载均会间接调用）。RemoveFlows用的是每个flow自己保存的calloutId，不依赖此id。
        }
    }
}
...
void StopWFP()
{
    ...
    //只有订阅了才退订：系统启动后BFE已运行时，StartWFP直接注册，并没有订阅，g_ChangeHandle为NULL。

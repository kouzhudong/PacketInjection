...
*/
{
    ...
    *pSize = 0;
    ...
    if (numBytes) {
        pBuffer = (BYTE *)ExAllocatePoolZero(NonPagedPool, numBytes, TAG);//ExAllocatePoolZero默认清零。
        if (NULL == pBuffer) {
...
NTSTATUS InboundInject(_In_ PPENDED_PACKET packet)
{
    ...
    HANDLE InjectionHandle = packet->isIPv6 ? g_IpPacket6_InjectionHandle_in : g_IpPacket4_InjectionHandle_in;
...
NTSTATUS OutboundInject(_In_ PPENDED_PACKET packet)
{
    NTSTATUS status = STATUS_SUCCESS;
    NET_BUFFER_LIST* clonedNetBufferList = NULL;
    HANDLE InjectionHandle = packet->isIPv6 ? g_IpPacket6_InjectionHandle_out: g_IpPacket4_InjectionHandle_out;
...
*/
{
    ...
    SentToUser = ExAllocatePoolZero(PagedPool, sizeof(NOTIFICATION), TAG);//ExAllocatePoolZero默认清零。
    if (NULL == SentToUser) {
...
VOID WorkThread(_In_ PVOID StartContext)
{
    ...
    for (;;) {
        ...
        packet = CONTAINING_RECORD(listEntry, PENDED_PACKET, listEntry);
        FreePendedPacket(packet);//在自旋锁外释放：FreePendedPacket会调Fwps*/释放池。
    }

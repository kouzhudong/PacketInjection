...
//保护与应用层在线通信(FltSendMessage)期间，ClientPort/UserProcess不被并发的PortDisconnect释放。
//CreateCommunicationPort初始化，PortConnect重置，PortDisconnect排空，工作线程发送前后acquire/release。
...
*/
{
    ...
    if (NULL != g_Data.Filter) {
        ...
        g_Data.Filter = NULL;//置空，避免失败清理路径(DriverEntry __finally)二次注销同一已销毁filter。
    }
...
)
{
    ...
    //重置rundown，使上一次断开(drain)后本次连接可再次acquire。须在发布ClientPort/UserProcess前。
...
*/
{
    ...
    //解引用前校验长度，避免对NULL或过短缓冲区取Command。

...
HANDLE g_IpPacket4_InjectionHandle_in;
HANDLE g_IpPacket4_InjectionHandle_out;
HANDLE g_IpPacket6_InjectionHandle_in;
HANDLE g_IpPacket6_InjectionHandle_out;
...
PPENDED_PACKET BuildPendPacket(_In_ const FWPS_INCOMING_VALUES* inFixedValues,
...
*/
{
    ...
    packet = ExAllocatePoolZero(NonPagedPool, sizeof(PENDED_PACKET), TAG);
    if (NULL == packet) {
    ...
    packet->isIPv6 = !LayerIsIPv4(inFixedValues->layerId);

    packet->injectionContext = injectionContext;

    packet->compartmentId = DEFAULT_COMPARTMENT_ID;
    if (FWPS_IS_METADATA_FIELD_PRESENT(inMetaValues, FWPS_METADATA_FIELD_COMPARTMENT_ID)) {
        packet->compartmentId = (COMPARTMENT_ID)inMetaValues->compartmentId;
    }
    ...
    packet->Direction = GetPacketDirectionForLayer(inFixedValues->layerId);
...
void NTAPI IpPacketClassifyFn(_In_ const FWPS_INCOMING_VALUES * pClassifyValues,
...
*/
{
    ...
    HANDLE g_IpPacket_InjectionHandle = NULL;
    FWP_DIRECTION packetDirection = GetPacketDirectionForLayer(pClassifyValues->layerId);
    ...
    if (isIPv6) {
        if (packetDirection == FWP_DIRECTION_INBOUND) {
            g_IpPacket_InjectionHandle = g_IpPacket6_InjectionHandle_in;
        } else {
            g_IpPacket_InjectionHandle = g_IpPacket6_InjectionHandle_out;
        }
    } else {
        if (packetDirection == FWP_DIRECTION_INBOUND) {
            g_IpPacket_InjectionHandle = g_IpPacket4_InjectionHandle_in;
        } else {
            g_IpPacket_InjectionHandle = g_IpPacket4_InjectionHandle_out;
        }
    }
...
VOID NTAPI FlowDeleteFn(IN UINT16 layerId, IN UINT32 calloutId, IN UINT64 flowContext)
{
    ...
    //必须用InterlockedDecrement返回值原子判0，否则“减完再读refCount”非原子。
    if (0 == InterlockedDecrement(&flowData->refCount)) {
        if (NULL != flowData->sid) {//sid可能为NULL(流无ALE_USER_ID)，ExFreePoolWithTag(NULL)会蓝屏。
                         ...
                         UINT32 calloutId)
{
    ...
    */
    ...
    //associate与insert须在锁内原子完成：关联成功后FlowDeleteFn可能立刻触发并对本节点做RemoveEntryList(持同一锁)。
    //若associate在锁外，存在“已关联但尚未入链”的窗口，FlowDeleteFn对未入链的listEntry{0,0}做RemoveEntryList会崩。
    KeAcquireInStackQueuedSpinLock(&g_flowContextListLock, &lockHandle);
...
void NTAPI EstablishedClassifyFn(_In_ const FWPS_INCOMING_VALUES * pClassifyValues,
                                 _In_ const FWPS_INCOMING_METADATA_VALUES * pMetadata,
                                 _Inout_opt_ void * layerData,
                                 _In_opt_ const void * classifyContext,
...
)
{
    ...
    */
    ...
    case FWPS_LAYER_ALE_FLOW_ESTABLISHED_V4:
        ...
        AssociateOneContext(pClassifyValues, pMetadata, FWPS_LAYER_INBOUND_IPPACKET_V4, g_CallOutId.INBOUND_IPPACKET_V4);
        AssociateOneContext(pClassifyValues, pMetadata, FWPS_LAYER_OUTBOUND_IPPACKET_V4, g_CallOutId.OUTBOUND_IPPACKET_V4);
    ...
    case FWPS_LAYER_ALE_FLOW_ESTABLISHED_V4_DISCARD://看看走到这里过没？
        ...
        AssociateOneContext(pClassifyValues, pMetadata, FWPS_LAYER_INBOUND_IPPACKET_V4_DISCARD, g_CallOutId.INBOUND_IPPACKET_DISCARD_V4);
        AssociateOneContext(pClassifyValues, pMetadata, FWPS_LAYER_OUTBOUND_IPPACKET_V4_DISCARD, g_CallOutId.OUTBOUND_IPPACKET_DISCARD_V4);
    ...
    case FWPS_LAYER_ALE_FLOW_ESTABLISHED_V6:
        ...
        AssociateOneContext(pClassifyValues, pMetadata, FWPS_LAYER_INBOUND_IPPACKET_V6, g_CallOutId.INBOUND_IPPACKET_V6);
        AssociateOneContext(pClassifyValues, pMetadata, FWPS_LAYER_OUTBOUND_IPPACKET_V6, g_CallOutId.OUTBOUND_IPPACKET_V6);
    ...
    case FWPS_LAYER_ALE_FLOW_ESTABLISHED_V6_DISCARD://看看走到这里过没？
        ...
        AssociateOneContext(pClassifyValues, pMetadata, FWPS_LAYER_INBOUND_IPPACKET_V6_DISCARD, g_CallOutId.INBOUND_IPPACKET_DISCARD_V6);
        AssociateOneContext(pClassifyValues, pMetadata, FWPS_LAYER_OUTBOUND_IPPACKET_V6_DISCARD, g_CallOutId.OUTBOUND_IPPACKET_DISCARD_V6);
...
void DestroyInjectionHandle()
{
    ...
    if (g_IpPacket4_InjectionHandle_in != NULL) {
        status = FwpsInjectionHandleDestroy(g_IpPacket4_InjectionHandle_in);
        ASSERT(NT_SUCCESS(status));
        g_IpPacket4_InjectionHandle_in = 0;
    }

    if (g_IpPacket4_InjectionHandle_out != NULL) {
        status = FwpsInjectionHandleDestroy(g_IpPacket4_InjectionHandle_out);
        ASSERT(NT_SUCCESS(status));
        g_IpPacket4_InjectionHandle_out = 0;
    }

    if (g_IpPacket6_InjectionHandle_in != NULL) {
        status = FwpsInjectionHandleDestroy(g_IpPacket6_InjectionHandle_in);
        ASSERT(NT_SUCCESS(status));
        g_IpPacket6_InjectionHandle_in = 0;
    }

    if (g_IpPacket6_InjectionHandle_out != NULL) {
        status = FwpsInjectionHandleDestroy(g_IpPacket6_InjectionHandle_out);
        ASSERT(NT_SUCCESS(status));
        g_IpPacket6_InjectionHandle_out = 0;
    }
}
...
void StopWFP()
{
    ...
    //BFE已运行时StartWFP直接注册、未订阅，g_ChangeHandle为NULL；只有订阅了才退订。置空使StopWFP幂等。
...
CALLOUT_FILTER g_CalloutFilter[] =
{
    ...
    //注册TRANSPORT相关的处理。
    {&FWPM_LAYER_INBOUND_IPPACKET_V4,              &g_CallOutId.INBOUND_IPPACKET_V4,          IpPacketClassifyFn,  NotifyFn,   FlowDeleteFn},
    {&FWPM_LAYER_INBOUND_IPPACKET_V6,              &g_CallOutId.INBOUND_IPPACKET_V6,          IpPacketClassifyFn,  NotifyFn,   FlowDeleteFn},
    {&FWPM_LAYER_OUTBOUND_IPPACKET_V4,             &g_CallOutId.OUTBOUND_IPPACKET_V4,         IpPacketClassifyFn,  NotifyFn,   FlowDeleteFn},
    {&FWPM_LAYER_OUTBOUND_IPPACKET_V6,             &g_CallOutId.OUTBOUND_IPPACKET_V6,         IpPacketClassifyFn,  NotifyFn,   FlowDeleteFn},
    //{&FWPM_LAYER_INBOUND_IPPACKET_V4_DISCARD,      &g_CallOutId.INBOUND_IPPACKET_DISCARD_V4,  IpPacketClassifyFn,  NotifyFn,   FlowDeleteFn},
    //{&FWPM_LAYER_INBOUND_IPPACKET_V6_DISCARD,      &g_CallOutId.INBOUND_IPPACKET_DISCARD_V6,  IpPacketClassifyFn,  NotifyFn,   FlowDeleteFn},
...
NTSTATUS CreateInjectionHandle()
{
    ...
    NtStatus = FwpsInjectionHandleCreate(AF_INET, FWPS_INJECTION_TYPE_NETWORK, &g_IpPacket4_InjectionHandle_in);
    if (!NT_SUCCESS(NtStatus)) {
        PrintEx(DPFLTR_IHVNETWORK_ID, DPFLTR_ERROR_LEVEL, "错误：status:%#x", NtStatus);
        return NtStatus;
    }

    NtStatus = FwpsInjectionHandleCreate(AF_INET6, FWPS_INJECTION_TYPE_NETWORK, &g_IpPacket6_InjectionHandle_in);
    if (!NT_SUCCESS(NtStatus)) {
        PrintEx(DPFLTR_IHVNETWORK_ID, DPFLTR_ERROR_LEVEL, "错误：status:%#x", NtStatus);
        return NtStatus;
    }

    NtStatus = FwpsInjectionHandleCreate(AF_INET, FWPS_INJECTION_TYPE_NETWORK, &g_IpPacket4_InjectionHandle_out);
    if (!NT_SUCCESS(NtStatus)) {
        PrintEx(DPFLTR_IHVNETWORK_ID, DPFLTR_ERROR_LEVEL, "错误：status:%#x", NtStatus);
        return NtStatus;
    }

    NtStatus = FwpsInjectionHandleCreate(AF_INET6, FWPS_INJECTION_TYPE_NETWORK, &g_IpPacket6_InjectionHandle_out);
    if (!NT_SUCCESS(NtStatus)) {
        PrintEx(DPFLTR_IHVNETWORK_ID, DPFLTR_ERROR_LEVEL, "错误：status:%#x", NtStatus);
        return NtStatus;
    }

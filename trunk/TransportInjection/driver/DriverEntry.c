...
void * gWorkerThread;//工作线程对象。单线程串行处理absorb的包，避免多线程乱序重注入。
...
*/
{
    ...
    //之后再拆WFP和通信端。否则worker会用已释放的filter/注入句柄，造成use-after-free(原顺序是先Unload/StopWFP后等线程，是错的)。
    if (NULL != gWorkerThread) {
        KeWaitForSingleObject(gWorkerThread, Executive, KernelMode, FALSE, NULL);
        ObDereferenceObject(gWorkerThread);
        gWorkerThread = NULL;
    }
...
NTSTATUS DriverEntry(_In_ PDRIVER_OBJECT pDriverObject, _In_ PUNICODE_STRING pRegistryPath)
{
    ...
    __try {
        ...
        status = ObReferenceObjectByHandle(threadHandle, 0, NULL, KernelMode, &gWorkerThread, NULL);
